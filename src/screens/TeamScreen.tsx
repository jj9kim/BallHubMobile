...
function RankBadge({ rank }: { rank?: number }) {
  if (!rank) return null;
  return (
    <View style={{ backgroundColor: '#2a2a2a', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, marginLeft: 8 }}>
      <Text style={{ color: '#aaa', fontSize: 10, fontWeight: '600', letterSpacing: 0.3 }}>#{rank}</Text>
    </View>
  );
}
            ...
            <View style={{ flexDirection: 'row' }}>
              {[
                ...
                <View key={label} style={[s.bigStatBox, i > 0 && { borderLeftWidth: 1, borderLeftColor: '#2a2a2a' }]}>
                  ...
                  <Text style={s.bigStatLbl}>{label}</Text>
                  {ranks[rankKey] && <Text style={{ color: '#aaa', fontSize: 10, fontWeight: '600', marginTop: 2, letterSpacing: 0.3 }}>#{ranks[rankKey]}</Text>}
                </View>
              ))}
            </View>
          </View>
